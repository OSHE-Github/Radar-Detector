% Tutorials / horn antenna - MODIFIED FOR KA-BAND (26.5 - 40 GHz)
%
% Description at:
...
% -------------------------------------------------------------------------
% 2. KA-BAND CONSTRAINTS (WR-28 Standard)
% -------------------------------------------------------------------------
% Waveguide width (Dimension 'a' for WR-28)
horn.width  = 7.112; 
% Waveguide height (Dimension 'b' for WR-28)
horn.height = 3.556; 

% Horn Physical Dimensions
horn.length = 50;       % Flare length
horn.feed_length = 15;  % Feed length (Shortened for higher freq)
horn.thickness = 1;     % Thinner walls for smaller scale

% Horn flare angle (approx 15-20 deg is standard)
horn.angle = [15 15]*pi/180;

% Simulation Box (Reduced size for Ka-Band to save time)
% 200mm was too big; 100mm is sufficient for these small wavelengths
SimBox = [100 100 100]; 

% Frequency range of interest (Ka-Band)
f_start =  26.5e9;
f_stop  =  40.0e9;

% Frequency of interest for plotting (Center of Ka-Band)
f0 = 33.25e9;
...
% CHANGED: Increased resolution divisor to 20 for higher accuracy at 40GHz
...
% horn aperture (Calculation for Directivity)
A = (a + 2*sin(horn.angle(1))*horn.length)*unit * (b + 2*sin(horn.angle(2))*horn.length)*unit;

%% apply the excitation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
start=[-a/2 -b/2 mesh.z(8) ]; % mesh.z(8) ensures we are inside the feed
...
ylim([-60 0]);
grid on
title( 'Reflection Coefficient S_{11} (Ka-Band)' );
...
title(['Far Field Pattern @ ' num2str(f0/1e9) ' GHz']);
drawnow

% polar plot
figure
polarFF(nf2ff,'xaxis','theta','param',[1 2],'logscale',[-40 20], 'xtics', 12);
title(['Polar Pattern @ ' num2str(f0/1e9) ' GHz']);
drawnow
